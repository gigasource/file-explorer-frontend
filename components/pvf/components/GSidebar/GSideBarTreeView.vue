...
export default {
  ...
  setup(props, context) {
    ...
    const genNodeContent = function ({node, text, childrenVNodes, path, isLast}) {
      ...
      const appendIcon = node.appendIcon && <g-icon small="true" class="mx-1">{node.appendIcon}</g-icon>
      ...
      const fallbackContent = <li class={!treeStates[path].collapse && childrenVNodes && 'g-treeview__open'}>
        ...
        {children ? <g-expand-transition>{children}</g-expand-transition> : <div/>}
      </li>
    ...
    const itemTextFn = (typeof props.itemText === 'function' && props.itemText) || ((node) => {
